...
class FiveLevelEncoder(nn.Module):
    def __init__(self, nc, latent_dims):
        super(FiveLevelEncoder, self).__init__()
        self.nc = nc
        self.latent_dims = latent_dims
        self.encoder = nn.Sequential(
            nn.Conv2d(self.nc, 64, 4, 2, padding="valid"),  # 1          # B,  32, 32, 64
        ...
        self.third_latents = nn.Sequential(
                        ...
                        nn.LeakyReLU(0.3),
                        nn.Linear(self.latent_dims[1] * 2, self.latent_dims[2] * 2))
        self.forth_latents = nn.Sequential(
                        nn.Linear(self.latent_dims[2] * 2, self.latent_dims[2] * 2),
                        nn.LeakyReLU(0.3),
                        nn.Linear(self.latent_dims[2] * 2, self.latent_dims[3] * 2))
        self.fifth_latents = nn.Sequential(
                        nn.Linear(self.latent_dims[3] * 2, self.latent_dims[3] * 2),
                        nn.LeakyReLU(0.3),
                        nn.Linear(self.latent_dims[3] * 2, self.latent_dims[4] * 2))
    ...
    def forward(self, x):
        ...
        second_latent = self.second_latents(first_latent)
        third_latent = self.third_latents(second_latent)
        forth_latent = self.forth_latents(third_latent)
