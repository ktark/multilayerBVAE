...
def main(hparams):
    ...
    wandb_logger = WandbLogger(
        name=f'Hierarchy: ds {hparams.dataset} | '
             ...
             f'lr: {hparams.lr}',
        project='thesis', job_type='train', log_model=True)

    wandb_logger.watch(vae, log_freq=1000)  # log network topology and weights
    epoch_end_example_image_S = ImagePredictionLogger(sample=15, ds=ds, wandb_logger=wandb_logger)  # Square
    epoch_end_example_image_s2 = ImagePredictionLogger(sample=4, ds=ds, wandb_logger=wandb_logger)  # Square
    test_image400 = ImagePredictionLoggerHierarchy(sample=4, ds=ds, wandb_logger=wandb_logger)
    test_image6 = ImagePredictionLoggerHierarchy(sample=15, ds=ds, wandb_logger=wandb_logger)
